...
| **修改** | `src/llm/client.py` | timeout + 流式修复 | AsyncOpenAI 超时拆分 + 流式 max_tokens + delta 空值防护 |
| **修改** | `src/infra/prompt_manager.py` | +1 方法 | `get_prompt_config()` 供流式端点使用 |
| **修改** | `src/core/generation.py` | 历史 token 截断 | 会话历史注入时增加 token 预算保护 |
| **修改** | `prompts/v1/answer_generation.yaml` | prompt 优化 | 增加元问题/会话历史处理规则 |
| **新增** | `requirements.txt` | +1 依赖 | `duckduckgo_search>=3.5` |

总计：新增 ~2600 行代码，修改 ~400 行。

---

## 5. Phase 2：Agent 流式化与工具扩展

### 5.1 Agent 流式对话（SSE）

在原有同步 `POST /agent/chat` 基础上，新增 `GET /agent/chat/stream` SSE 端点。
...
data: {"chunk": " the"}
...
data: {"done": true, "answer": "...", "iterations": 2, "tool_calls": [...]}
```

#### 异步核心循环

`AgentHarness.run_async_stream()` 将同步 Plan-Execute-Observe 循环改造为 `AsyncGenerator`：
- 每个 planning → yield `{"step": "planning"}`
- 每个 tool_call → yield `{"step": "tool_call"}`
- 每个 tool_done → yield `{"step": "tool_done"}`
- 最终答案按空白字符拆分 → 逐词 yield `{"chunk": "..."}`
- 结束 → yield `{"done": true}`

#### 异步工具执行

- `search_knowledge_base` → `await flow.run_async()`（真正异步）
- 其他工具（calculator、get_weather、search_web）→ `asyncio.to_thread()`（线程池，不阻塞事件循环）

### 5.2 新增工具

| 工具 | 数据源 | 功能 | 安全 |
|---|---|---|---|
| `get_weather` | [wttr.in](https://wttr.in) 免费 API | 查询城市实时天气（温度/湿度/风速） | WHITELIST |
| `search_web` | DuckDuckGo（优先 SDK，5s 超时后 HTML fallback） | 搜索互联网获取最新信息 | GRAYLIST |

#### search_web 可靠性设计

- **优先路径**：`duckduckgo_search` Python SDK（v3.x 兼容）
- **5 秒超时**：用 `ThreadPoolExecutor` 包装，防止网络问题卡死 Agent
- **HTML fallback**：解析 `lite.duckduckgo.com`，正则提取标题/摘要/URL，并将 DDG 重定向 URL 还原为真实目标地址
- **HTML 实体清理**：`&nbsp;` → 空格、`ud=...` 重定向解码

### 5.3 Agent Web UI（`GET /agent`）

独立的纯内联 HTML 页面，零外部依赖：

- **输入区**：消息输入框 + 发送/停止按钮
- **思考过程区**：实时展示 Agent 每一步（🤔 规划中 → 🔧 调用工具 → ✅/❌ 完成），工具参数 JSON 可点击展开/折叠
- **答案区**：rAF 逐词打字效果 + 完成后 URL 自动转为可点击链接
- **停止按钮**：`⏹ 停止` 中断当前 SSE 连接
- **会话管理**：localStorage 持久化 session_id，🔄 新会话清除历史
- **页面跳转**：顶部链接可在 RAG 搜索（`/`）和 Agent 模式（`/agent`）之间切换

### 5.4 RAG 流式页面增强

- **停止按钮**：搜索进行中显示红色 `⏹ 停止`，点击关闭 SSE 连接 + 恢复 UI
- **Agent 模式入口**：页面顶栏新增 `🤖 Agent 模式` 链接
- **结果容器提前可见**：流式开始前即 `display: block`，确保 rAF 逐词更新可被用户感知

### 5.5 基础设施修复

| 修复 | 文件 | 问题 | 方案 |
|---|---|---|---|
| AsyncOpenAI 超时 | `src/llm/client.py` | `timeout=60.0` 覆盖连接+流式读取总时长 | 拆分为 `Timeout(connect=15, read=180, write=60, pool=10)` |
| 流式 max_tokens | `src/llm/client.py` | 默认 None → 无限制生成 | 默认 1024（与 YAML prompt 对齐） |
| 流式 delta 空值 | `src/llm/client.py` | `delta=None` 时抛 AttributeError | 增加 `if delta is None` 防护 |
| 工具结果格式化 | `src/agent/harness.py` | Python dict 字符串对 LLM 不友好 | 新增 `_format_tool_result()`——search_web 编号+URL，weather 一行摘要 |
| Planner prompt | `src/agent/harness.py` | 不知道何时用 weather/search_web | 新增规则：weather→get_weather，实时信息→search_web，URL 必须列出 |
| 会话历史超窗口 | `app.py` + `generation.py` | 历史无条件全部注入 | token 预算保护（最多 40% 上下文窗口） |
